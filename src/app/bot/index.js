...
const {Vue, Vuex, jssc} = require('../../modules/libs')
...
module.exports = function ({functions, lists, display, displayLog, nextNode}) {
  ...
  const ui = new Bot(functions, lists)
...
class Bot {
  constructor(functions, lists, mainDiv) {
    ...
    const fdiv = this.makeFunctionUI(functions)

    this.elems = fdiv ? [fdiv, this.makeConfUI(lists)] : []
  }

  makeFunctionUI(functions) { // eslint-disable-line max-lines-per-function,max-statements
    let plugin
    const self = this
    ...
    if (ffunctions.length) {
      selectFunctions.addEventListener('change', ((fffunctions) => function () {
        ...
        plugin = fffunctions[id]
        // [1] Note that this must also be executed immediately,
        //   see [2]
        const makeUI = plugin.makeBotUI
      ...
      this.configuration.function_ = (title, callContinue, chainArgs) => plugin.main_bot(title, callContinue, chainArgs)
...
currently running bots`
  ...
  _enableStopBot(stopId) {
    ...
    stop.addEventListener('click', ((id) => function () {
      ...
      return App.log.info('Bot stopped manually')
    }
    )(stopId), false)
  ...
  _previewFilter() { // eslint-disable-line max-lines-per-function,max-statements
    ...
    if (this.makeFilters()) {
      if (this.configuration.list.current[0].nodeName === 'TBODY') {
        items =
                    this.configuration.list.current[0].getElementsByTagName('td')
      } else {
        items =
...
a browser page refresh`)
      ...
      App.log.hidden(`Plugin: ${this.configuration.plugin_name}`)
      App.log.hidden(`Filter: ${document.getElementById('WikiMonkeyBotFilter').value}`)
  ...
  _processItem(status, items, index, linkId, chainArgs) { // eslint-disable-line max-lines-per-function,max-statements,max-params
    ...
    if (items[index]) {
      ...
      if (link && this.canProcessPage(link)) {
        ...
        App.log.info(`Waiting ${interval / 1000}` + ' seconds ...')
